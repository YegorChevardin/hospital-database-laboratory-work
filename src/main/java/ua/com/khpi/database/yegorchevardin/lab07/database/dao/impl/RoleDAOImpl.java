...
public class RoleDAOImpl implements RoleDAO {
    private static final String FIND_BY_ID =
            "SELECT * FROM " + DatabaseConstants.ROLES_TABLE.getValue()
            + " WHERE id = ?";
    private static final String FIND_ALL =
            "SELECT * FROM " + DatabaseConstants.ROLES_TABLE.getValue();
    private static final String INSERT_ROLE =
            "INSERT INTO " + DatabaseConstants.ROLES_TABLE.getValue() + "(name) VALUES (?)";
    private static final String DELETE_BY_ID =
            "DELETE FROM " + DatabaseConstants.ROLES_TABLE.getValue()
            + " WHERE id = ?";
    ...
    public Optional<Role> getById(Long id) {
        ...
        try (
            ...
            if (resultSet.next()) {
                role = extractFromResultSet(resultSet);
            }
    ...
    public List<Role> getAll() {
        ...
        try (
            ...
            while (resultSet.next()) {
                roles.add(extractFromResultSet(resultSet));
            }
        } catch (SQLException e) {
            throw new DatabaseException(e.getMessage(), e);
        }
